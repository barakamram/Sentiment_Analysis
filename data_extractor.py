...
class AudioExtractor:
    ...
    def __init__(self, audio_config=None, verbose=1, features_folder_name="features", classification=True,
                    emotions=["NEUTRAL", "POSITIVE", "NEGATIVE"], balance=True):
    ...
    def load_metadata_from_desc_file(self, desc_files, partition):
        ...
        if not self.classification:
            ...
            if len(self.emotions) == 3:
                self.categories = {'POSITIVE': 1, 'NEUTRAL': 2, 'NEGATIVE': 3}
                # self.categories = {'sad': 1, 'neutral': 2, 'happy': 3}
            elif len(self.emotions) == 6:
                self.categories = {'angry': 1, 'sad': 2, 'neutral': 3, 'ps': 4, 'happy': 5, 'exited': 6}
            elif len(self.emotions) == 9:
                self.categories = {'angry': 1, 'sad': 2, 'neutral': 3, 'ps': 4, 'happy': 5, 'boredom': 6, "disgust": 7, "calm": 8, "fear": 9}
            else:
        ...
        if os.path.isfile(name):
            ...
            # from: https://huggingface.co/speechbrain/spkrec-xvect-voxceleb
            ...
            for audio_file in tqdm.tqdm(audio_paths, f"Extracting features for {partition}"):
                ...
                signal, fs = torchaudio.load(audio_paths[num_of_audio_file])
                # classifier.encode_batch(signal) --> tensor type: tensor([[[ ... ]]])
                # detach().cpu().numpy() --> numpy type: [[[ ... ]]]
                embeddings = classifier.encode_batch(signal).detach().cpu().numpy()
                feature = np.concatenate((feature, embeddings[0][0]))
                append(feature)
                num_of_audio_file += 1
    ...
    def _balance_data(self, partition):
        ...
        if minimum == 0:
            ...
            # print(f"[!] One class has 0 samples, setting balance to False")
            self.balance = False
            return
        if self.verbose:
...
def do_pca(train, test):
    pca = pca_algo(train, test)
    pca.do_pca()
    return pca.train, pca.test
...
def load_data(train_desc_files, test_desc_files, audio_config=None, classification=True, shuffle=True,
                balance=True, emotions=['POSITIVE', 'NEUTRAL', 'NEGATIVE']):
    ...
    #######################
    train_features, test_features = do_pca(audiogen.train_features, audiogen.test_features)
    #######################
