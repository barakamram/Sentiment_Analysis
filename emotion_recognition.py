...
class EmotionRecognizer:
    ...
    def __init__(self, model=None, **kwargs):
        ...
        """
        # emotions
        self.emotions = kwargs.get("emotions", ["NEUTRAL", "NEGATIVE", 'POSITIVE'])
    ...
    def train(self, verbose=1):
        ...
        if not self.model_trained:
            ...
            print(f"[#] Duration training time: {end - start}")
            if verbose:
                print("[+] Model trained")
    ...
    def predict_proba(self, audio_path):
        ...
        if self.classification:
            print(f"predicting wav file: {audio_path}")
...
def do_pca_predict_proba(predict=None):
    pca = pca_algo(predict)
    pca.do_pca_predict_proba()
    return pca.predict,
